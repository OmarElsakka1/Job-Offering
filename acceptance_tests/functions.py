...
class LoginActionTesting():
    ...
    def do_change_password_action(self, oldpass, newpass, msg, page, button):
        if (self.__make_sure_log_in()):
            return ChangePassword().do_change_password(
                self.driver, oldpass, newpass, msg, page, button)
        else:
            return False

    def do_change_profile_action(
            self,
            newemail=None,
            firstname=None,
            lastname=None,
    ...
    def do_logout_action(self):
        if (self.__make_sure_log_in()):
            return LogoutTesting().do_logout(self.driver)  # action
        else:
            return False
